...
#define _HI(p)      HAL_GPIO_WritePin(p ## _GPIO_Port, p ## _Pin, GPIO_PIN_SET)
#define _LO(p)      HAL_GPIO_WritePin(p ## _GPIO_Port, p ## _Pin, GPIO_PIN_RESET)

#define _HI_RS()    _HI(RES)
#define _LO_RS()    _LO(RES)
#define _HI_DC()    _HI(DC)
#define _LO_DC()    _LO(DC)
#define _HI_CS()    _HI(SPI1_SS)
#define _LO_CS()    _LO(SPI1_SS)
...
static HAL_StatusTypeDef spi_transfer(uint8_t* buffer, uint16_t size)
{
    HAL_StatusTypeDef status;

    spi1_transmit_complete = false;
    status = HAL_SPI_Transmit_DMA(&hspi1, buffer, size);
    // status = HAL_SPI_Transmit(&hspi1, buffer, size, 100);
    while (!spi1_transmit_complete) {
        __WFE();
    }
    return status;
}

__STATIC_INLINE HAL_StatusTypeDef i2c_register_write(uint8_t* buffer, uint16_t size)
{
    return HAL_I2C_Master_Transmit(&hi2c1, _i2caddr, buffer, size, 1000);
    /*
...
__STATIC_INLINE HAL_StatusTypeDef i2c_memory_transfer(uint8_t* buffer, uint16_t size)
{
    static uint8_t control = 0x40;
    return HAL_I2C_Mem_Write(&hi2c1, _i2caddr, control, I2C_MEMADD_SIZE_8BIT, buffer, size, 1000);
}
...
void ssd1306_command(uint8_t c)
{
    if (use_i2c) {
        ...
        ret = i2c_register_write(dta_send, 2);
        UNUSED_VARIABLE(ret);
    }
    else {
        _HI_CS();
...
void ssd1306_display(void)
{
    ...
    if (use_i2c) {
        ...
        i2c_memory_transfer(buffer, (SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8));
    }
    else {
        _HI_CS();
        _HI_DC();
        ...
        */
        UNUSED_VARIABLE(spi_transfer(buffer, (SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8)));
        _HI_CS();
    }
}
